...
This queries the live wallpaper state at snapshot time using the detected backend (swww or hyprpaper).
  ...
  <verify>
Run `cargo check -p vulcan-appearance-manager` - no errors, method compiles
  </verify>
  <done>create_preview_snapshot() method exists and returns PreviewSnapshot with current wallpapers and theme_id</done>
</task>
