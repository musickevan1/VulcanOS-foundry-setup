...
<task type="auto">
  ...
  <action>
Add a new output message that App can use to query whether ThemeView is in preview state and get the previewing theme ID:

Option A: Add output message for close-time query
Option B: Track preview state in App coordinator (simpler)

Choose Option B: Track preview state in App via existing ThemeApplied messages.

Actually, we need to know TWO things at close time:
1. Is ThemeView in Previewing state?
2. What theme is being previewed?

The cleanest approach is to add an output message that ThemeView emits whenever preview state changes:
...
Emit this in ThemeSelected handler after state changes:
```rust
// After transitioning to Previewing in ThemeSelected:
sender.output(ThemeViewOutput::PreviewStateChanged {
    is_previewing: true,
    theme_id: self.previewing_theme_id.clone(),
}).ok();

// After transitioning to Idle in CancelPreview and ApplyTheme:
sender.output(ThemeViewOutput::PreviewStateChanged {
    is_previewing: false,
    theme_id: None,
}).ok();
...
Import theme_applier at top of app.rs:
```rust
use crate::services::theme_applier;
```
  </action>
...
1. AppState state machine wired into ThemeViewModel
2. Action bar with Revealer animation (slides in during preview)
3. Cancel restores original theme AND wallpapers
4. Apply transitions through Applying state with spinner
5. Implicit apply when closing app during preview
  </what-built>
  <how-to-verify>
...
7. Test button sensitivity:
   - While NOT previewing: Apply/Cancel buttons should NOT be visible
