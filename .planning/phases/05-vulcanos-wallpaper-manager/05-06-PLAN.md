...
must_haves:
  ...
  key_links:
    ...
    - from: "vulcan-wallpaper-manager/src/app.rs"
      to: "vulcan-wallpaper-manager/src/components/split_dialog.rs"
      via: "Controller instantiation on ShowSplitDialog"
      pattern: "SplitDialogOutput::Generated"
---
...
<task type="auto">
  ...
  <action>
Add "Import Panoramic" action to the app header and properly integrate the split dialog component.

1. Add import for split_dialog component:
```rust
use crate::components::split_dialog::{SplitDialogModel, SplitDialogInput, SplitDialogOutput};
```

2. Add split dialog messages to AppMsg:
```rust
#[derive(Debug)]
pub enum AppMsg {
    ...
    RefreshMonitors,
    OpenDirectory,
    // Profile management messages (from 05-05)
...
3. Add split_dialog Controller (Option since it's created on-demand) to App struct:
```rust
pub struct App {
    monitors: Vec<Monitor>,
...
4. Add import button to header bar (in the view! macro, after the folder button):
```rust
...
5. In update(), implement ShowSplitDialog to create the dialog as a popup window:
```rust
...
7. Initialize split_dialog fields to None in init():
```rust
let model = App {
    monitors,
...
cd vulcan-wallpaper-manager && cargo build
  ...
  <done>App compiles with fully integrated split dialog that opens as popup, applies wallpapers, and closes on completion</done>
</task>
...
6. "Import panoramic" button appears in header bar
7. Clicking button opens split dialog as popup window
8. After splitting, wallpapers are applied and dialog closes
</verification>
...
- Dialog opens as modal popup window on button click
- Generated wallpapers are applied immediately via hyprpaper
- Dialog closes automatically on success or cancel
- Profile manager is notified of new wallpaper assignments
</success_criteria>
