use crate::display::fonts::{FontConfig, Pattern};
...
const USER_CFG_PATH: &str = "/etc/vulcanbar/config.toml";
...
fn load_config(width: u16) -> (Config, [FunctionLayer; 2]) {
    let mut base =
        toml::from_str::<ConfigProxy>(&read_to_string("/usr/share/vulcanbar/config.toml").unwrap())
            .unwrap();
