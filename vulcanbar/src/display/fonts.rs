...
/// Load a font by fontconfig pattern string (e.g., "JetBrainsMono Nerd Font:style=Bold")
/// Returns a Cairo FontFace for use with set_font_face()
pub fn load_font_face(font_pattern: &str) -> Result<cairo::FontFace, FontConfigError> {
    let fontconfig = FontConfig::new();
    let mut pattern = Pattern::new(font_pattern);
    ...
    let ft_library = freetype::Library::init()
        .map_err(|_| FontConfigError::FontNotFound)?;
    let face = ft_library.new_face(file_name, file_idx)
        .map_err(|_| FontConfigError::FontNotFound)?;

    cairo::FontFace::create_from_ft(&face)
        .map_err(|_| FontConfigError::FontNotFound)
}
